...
| `kind` | what the app is, in one phrase (`walking game`, `counter + virtual dog`) — carried by rows from the `cute` guide, the only table with that column; an empty cell means the row is not in that guide |
